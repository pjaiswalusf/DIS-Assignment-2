...
namespace ISM6225_Spring_2024_Assignment_2
{
    class Program
    {
        ...
        */
        ...
        public static int RemoveDuplicates(int[] nums)
        {
            try
            {
                // Checks if the array has any value in it. Returns 0 if empty
                ...
                {
                    // If the current element is different from the previous one, store it at the current uniqueIndex
                    if (nums[i] != nums[i - 1])
                    {
        ...
        */
        ...
        public static IList<IList<int>> ThreeSum(int[] nums)
        {
            try
            {
                ...
                // Iterate through the array, stop at n - 2 to ensure at least two more elements remain
                // and the current element can be the first element in a triplet
                for (int i = 0; i < n - 2 && nums[i] <= 0; ++i)
                {
                    ...
                    // Two pointers approach to find the other two elements in the triplet
                    int j = i + 1, k = n - 1;
                    while (j < k)
                    {
    ...
    */
    ...
    public static int FindMaxConsecutiveOnes(int[] nums)
    {
        try
        {
            // Initialize variables to store the maximum count of consecutive ones and the current count
            int maxCount = 0;
            int count = 0;
            ...
            foreach (int num in nums)
            {
                ...
                else // If the current element is not 1 (i.e., 0), reset the count
                {
                    count = 0;
                }
            }
        ...
        */
        ...
        public static int BinaryToDecimal(int binary)
        {
            try
            {
                // Initialize variables to store decimal value and base value for binary conversion
                int decimalValue = 0; // Initialize the decimal value to be calculated
                int baseValue = 1; // Initialize the base value for binary conversion

                // Loop through each digit of the binary number until there are no more digits
                while (binary > 0)
                {
                    // Get the rightmost digit of the binary number
                    int remainder = binary % 10;

                    // Calculate decimal value by multiplying the remainder with the base value and adding to the total
                    decimalValue += remainder * baseValue;

                    // Move to the next digit of the binary number by dividing it by 10
                    binary /= 10;

                    // Update the base value for binary conversion by multiplying it by 2
                    baseValue *= 2;
                }
        ...
        */
        ...
        public static int MaximumGap(int[] nums)
        {
            try
            {
                ...
                // Arrays to store the minimum and maximum value in each bucket
                int[] minBucket = new int[bucketCount];
                int[] maxBucket = new int[bucketCount];

                // Initialize minBucket and maxBucket
        ...
        */
        ...
        public static string RemoveOccurrences(string s, string part)
        {
            try
            {
                bool bFlag = true; // Flag to control the loop
                while (bFlag == true) // Continue looping until bFlag is false
                {
                    if (s.Contains(part)) // Check if the string s contains the specified part
                    {
                        s = s.Replace(part, ""); // If it does, remove the occurrence of part from the string
                    }
                    else
                    {
                        bFlag = false; // If part is not found, set bFlag to false to exit the loop
                    }
                }
            ...
            {
                throw; // If any exception occurs, rethrow it
            }
        }
